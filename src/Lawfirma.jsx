...
const heroSlides = [
 ...
 {title:'Case Studies',eyebrow:'Explore more on our',description:'Click below to see how we deliver real impact for our clients.',image:'/assets/tnr-case-studies.png',alt:'TNR Attorneys reviewing case documents',cta:'Our Cases',to:'https://www.tnrattorneys.co.za/casestudies/'}
];
function HeroCarousel(){
 ...
 const touch=useRef(null),elapsed=useRef(0);const [progress,setProgress]=useState(0);
 ...
 useEffect(()=>{if(paused||interacting||reduced)return;let last=performance.now(),frame;const tick=now=>{const delta=now-last;last=now;if(!document.hidden){elapsed.current+=Math.min(delta,100);if(elapsed.current>=5000){elapsed.current=0;setActive(index=>(index+1)%heroSlides.length);}setProgress(elapsed.current/5000);}frame=requestAnimationFrame(tick);};frame=requestAnimationFrame(tick);return()=>cancelAnimationFrame(frame);},[paused,interacting,reduced]);
 const change=index=>{elapsed.current=0;setProgress(0);setActive((index+heroSlides.length)%heroSlides.length);setPaused(false);};
 ...
 return <section className="lf-hero lf-carousel" aria-label="TNR Attorneys introduction" aria-roledescription="carousel" onFocusCapture={()=>setInteracting(true)} onBlurCapture={e=>{if(!e.currentTarget.contains(e.relatedTarget))setInteracting(false);}} onTouchStart={e=>touch.current=e.touches[0].clientX} onTouchEnd={e=>{if(touch.current!==null){const distance=e.changedTouches[0].clientX-touch.current;if(Math.abs(distance)>50)change(active+(distance<0?1:-1));touch.current=null;}}}>
  <div className="lf-hero-frame">
   ...
   <div className="lf-carousel-controls" aria-label="Slideshow progress">{heroSlides.map((item,index)=><button key={item.title} aria-label={`Show slide ${index+1}: ${item.title}`} aria-pressed={active===index} onClick={()=>change(index)}><span className="lf-progress-track"><span className="lf-progress-fill" style={{transform:`scaleX(${index<active?1:index===active?(reduced?1:progress):0})`}}/></span></button>)}</div>
...
function TeamSection(){
 ...
 return <section className="lf-team-scroll" aria-labelledby="team-heading"><div className="lf-shell"><div className="lf-team-scroll-heading"><div><p className="lf-overview-eyebrow">Our People</p><h2 id="team-heading">Meet Our Team</h2><p className="lf-team-scroll-description">The attorneys, consultants and support team behind your matter.</p></div><div className="lf-team-scroll-actions"><Link to="/meettheteam/">About Our People<span className="lf-overview-chevron" aria-hidden="true"/></Link><div className="lf-team-scroll-buttons"><button aria-label="Previous team member" aria-controls="team-scroll-track" disabled={position.start} onClick={()=>move(-1)}><span aria-hidden="true"/></button><button aria-label="Next team member" aria-controls="team-scroll-track" disabled={position.end} onClick={()=>move(1)}><span aria-hidden="true"/></button></div></div></div><div className="lf-team-scroll-track" id="team-scroll-track" ref={track} onScroll={update} tabIndex="0" role="region" aria-label="All seven team members. Scroll horizontally to view more." onKeyDown={e=>{if(e.key==='ArrowRight'||e.key==='ArrowLeft'){e.preventDefault();move(e.key==='ArrowRight'?1:-1);}}}>{people.map(([name,role],index)=><figure key={name}><div className="lf-team-scroll-portrait"><img src={`/assets/${name.replaceAll(' ','-')}.png`} width="599" height="551" alt={name} loading="lazy"/><span aria-hidden="true">{String(index+1).padStart(2,'0')}</span></div><figcaption><h3>{name}</h3><p>{role}</p></figcaption></figure>)}</div><p className="lf-team-scroll-hint">Scroll to Meet Everyone</p></div></section>;
}
...
function CaseStudies(){return <section className="lf-case-studies" aria-labelledby="case-studies-heading"><div className="lf-shell"><p className="lf-overview-eyebrow">Our Work in Practice</p><h2 id="case-studies-heading">Case Studies</h2><div className="lf-case-grid">{caseStudies.map(([category,title,slug],index)=><a className="lf-case-card" key={slug} href={`https://www.tnrattorneys.co.za/portfolio/${slug}/`}><div className="lf-case-image"><img src={`/assets/case-study-${index+1}.png`} alt="" loading="lazy" width="700" height="600"/><span aria-hidden="true">0{index+1}</span></div><div className="lf-case-copy"><p>{category}</p><h3>{title}</h3><span className="lf-case-link">View Case Study <span aria-hidden="true">›</span></span></div></a>)}</div><p className="lf-case-note">Past outcomes relate to the circumstances of each case and do not guarantee similar results.</p></div></section>}
const clientNames=['Makhado Local Municipality','Ditsong Museums of South Africa','Department of Sports, Arts and Culture','Department of Justice and Constitutional Development','Municipal Client','Municipal Client','Public Protector South Africa','SITA','Pan South African Language Board','JB Marks Local Municipality','Industrial Development Corporation'];
